...
//added
//value to set the first and the last stair without changes
bool flval = false;  //disable by default
//value to set brightness for top/bottom
unsigned long flbrigh = 30;  
...
#define UPPER true
  ...
  //added
 bool fastled_colRed = 8 ;
 bool fastled_colGreen = 255 ; 
 bool fastled_colBlue = 0 ;

  void updateSegmentson() {
  ...
  void updateSegmentsoff() {
    mainSegmentId = strip.getMainSegmentId();
    WS2812FX::Segment mainsegment = strip.getSegment(mainSegmentId);
    WS2812FX::Segment* segments = strip.getSegments();
	WS2812FX::Segment& segef = strip.getSegment(0);
   WS2812FX::Segment& segel = strip.getSegment(10);
//test	
 bool fastled_colRed = 8 ;
 bool fastled_colGreen = 255 ; 
 bool fastled_colBlue = 0 ;
//test 
   ...
   for (int i = 0; i < MAX_NUM_SEGMENTS; i++, segments++) {
      ...
      if (i >= onIndex && i < offIndex) {
 strip.getSegment(i).setOption(SEG_OPTION_ON,  1, 1);
      } else {

 if (flval == true){
 if ((onIndex == 0 && offIndex == 0) or (onIndex == maxSegmentId && offIndex == maxSegmentId + 1 ))
  {
//test
//   segef.mode = FX_MODE_ANDROID ;
// segef.colors[0] = ((col[3] << 24) | ((fastled_colRed&0xFF) << 16) | ((fastled_colGreen&0xFF) << 8) | ((fastled_colBlue&0xFF)));
// segel.mode = FX_MODE_ANDROID ;
// segel.colors[0] = ((col[3] << 24) | ((fastled_colRed&0xFF) << 16) | ((fastled_colGreen&0xFF) << 8) | ((fastled_colBlue&0xFF)));
strip.setBrightness(flbrigh); 
...
if (flval == true){
// if (i != 0 && i != 10 ) {
 if (i != 0 && i != maxSegmentId ) {
       // segments->setOption(SEG_OPTION_ON, 0, 1);
       // segments[i].setOption(SEG_OPTION_ON, 0, 1);
        strip.getSegment(i).setOption(SEG_OPTION_ON,  0, 1);
       }

}else {strip.getSegment(i).setOption(SEG_OPTION_ON,  0, 1);}
 ...
 updateSegmentson();
     ...
     updateSegmentsoff();
	  ...
	  //added 2 new ones instead of below
 //updateSegments();
    }
  }
  ...
  void writeSettingsToJson(JsonObject& root) {
    ...
    //added flval flbrigh
    staircase["segment-first-last"] = flval;
    staircase["brightnes-firstlast"] = flbrigh;
  ...
  bool readSettingsFromJson(JsonObject& root) {
    ...
    //addded  flval 
 bool c_flval = staircase["segment-first-last"] | flval;
    if (c_flval != flval) {
      flval = c_flval;
      changed = true;
    } 

unsigned long c_flbrigh = staircase["brightnes-firstlast"] | flbrigh;
    if (c_flbrigh != flbrigh) {
      flbrigh = c_flbrigh;
      changed = true;
    } 
  ...
  void enable(bool enable) {
    if (enable) {
      ...
      Serial.println(" seconds.");
          //added
      Serial.print("segment-first-last: ");
      Serial.print(flval);
       Serial.print("brightnes-firstlast: ");
      Serial.print(flbrigh);
